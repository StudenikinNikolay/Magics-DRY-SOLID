...
public class Main {
    //Magic Numbers Principle - не используем значение параметра в коде напрямую, вместо чисел используем константы
    public static final char RUB = (char) 0x20BD;

    public static void main(String[] args) {
        //Liskov substitution principle - наследники класса Product играют роль предка
        Product[] products = {new Caviar(), new Pineapple(), new Kvass(), new Phone()};
        //принцип DRY - повторяющийся вывод списка продуктов на экран выносим в отдельный метод
        printProucts(products);
        Scanner scanner = new Scanner(System.in);
        while (true) {
            System.out.println("Введите номер товара и количество, разделенные знаком пробела, или введите `end` ");
